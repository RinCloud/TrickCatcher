...
int main() {
    ...
    for(i = 1; i < N; ++i) E[i] = Node{A[i].id, A[i + 1].id, A[i + 1].x - A[i].x}; // Modified this line and next line to correct the assignment of value to struct members.
    ...
    for(i = 1; i <= 2 * N - 2; ++i)
        if(gf(E[i].x) != gf(E[i].y))
            fa[gf(E[i].x)] = gf(E[i].y), ans += E[i].id; // Modified this line to correct the summation of costs.
    printf("%lld\n", ans);
    return 0;
}