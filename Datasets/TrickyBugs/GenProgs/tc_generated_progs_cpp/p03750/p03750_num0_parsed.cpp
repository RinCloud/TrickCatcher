...
int tg[maxn];
vector<int> v, vc0[maxn];
multiset<int> vc1[maxn];
...
int main() {
    ...
    priority_queue<pr, vector<pr>, greater<pr>> hp0;
    priority_queue<int> hp1;
    int ans = n;

    for (int i = m; i >= 1; --i) {
        for (int x : vc0[i]) hp0.push(pr(x, i));
        for (; tag[i] + tag[i + 1] < -1 && !hp0.empty(); hp0.pop()) {
            int x = hp0.top().first;
            ++tag[i];
            --tag[x - 1];
            --ans;
            ++add[i + 1];
            --add[hp0.top().second + 1];
            vc1[x].erase(vc1[x].find(hp0.top().second));
        }
        if ((tag[i] += tag[i + 1]) < -1) ans = -1e9;
    }
    ...
    for (int i = 1; i <= m; ++i) {
        ...
        for (; tag[i] + add[i] < 0 && !hp1.empty(); hp1.pop()) {
            ++add[i];
            --add[hp1.top() + 1];
            --ans;
        }
    ...
    rep(i, 1, q) {
        int x, y, ans;
        cin >> x >> y;
        ans = max(pre[find(x) - 1] + 1, pre[find(y) - 1]);
        cout << (ans < 0 ? -1 : ans) << '\n';
    }
}
