...
int main() 
{
	...
	for (int i=1;i<=n;i++)
	{
		...
		LL bs=-INF,bt=-INF;
		LL ns=INF,nt=INF;
		if (it1!=s.begin()) bs=*prev(it1); // Fix: Use prev() to find previous element
		if (it2!=s.end()) ns=*next(it2); // Fix: Use next() to find next element
		s.erase(it);
		t.insert(x);
		it=t.find(x);
		it1=it,it2=it;
		if (it1!=t.begin()) bt=*prev(it1); // Fix: Use prev() to find previous element
		if (it2!=t.end()) nt=*next(it2); // Fix: Use next() to find next element
		t.erase(it);
