...
struct BIT{
    ...
    inline void ad(int x,int vv){
        for (;x<=n;x+=(x&(-x))){
            tg[++cnt]=x;vl[cnt]=t[x];
            t[x]=min(t[x],vv); // changed from max to min
        }
    }
    inline int get(int x){
        int ans=1e9; // changed from -1e9 to 1e9
        for (x=min(x,n);x;x-=(x&(-x)))ans=min(ans,t[x]); // changed from max to min
        return ans;
    }
...
int main(){
    ...
    memset(bit[1].t,0x3f,sizeof(bit[1].t)); // changed from memset(bit[1].t,0x8f,sizeof(bit[1].t));
