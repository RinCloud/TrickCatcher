...
int main()
{
	...
	for(int R=k,B=0;R>=n && B<=k;R--,B++) // fixed loop termination condition
	{
		int T=R-n+1;
		if(T<0) break;
	...
	{
		int x=k>>1,T=x-n+1;
		if(T>=0) ans=((LL)ans+C(k-1,x)-C(k-1,x-1-T))%mo;
		else ans=((LL)ans+C(k-1,x)) % mo; // fixed condition for T < 0
	}
	printf("%d",ans);
	return 0;
}
