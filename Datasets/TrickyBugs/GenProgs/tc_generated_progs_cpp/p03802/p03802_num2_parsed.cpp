...
int id[80001], low[80001], dfn[80001], col[80001];

void build(int nod,int l,int r) {
    id[nod] = ++tot;
    if(nod > 1) {
        point[id[nod>>1]].push_back(tot);
    }
    if(l == r) {
        int v = vec[l-1].i;
...
void update(int nod,int l,int r,int a,int b,int x) {
    ...
    int mid = l + r >> 1;
    if(mid >= b) {
        update(nod<<1, l, mid, a, b, x);
    } else if(mid < a) {
        update(nod<<1|1, mid + 1, r, a, b, x);
    } else {
        update(nod << 1, l, mid, a, mid, x);
        update(nod<<1|1, mid + 1, r, mid + 1, b, x);
    }
}
...
pair<int,int> get(int i,int m) {
    ...
    int l = 1, r = i, mid;
    while(l <= r) {
        if(vec[i-1].x - vec[(mid = l + r >> 1) - 1].x >= m) {
            l = mid + 1;
        } else {
            r = mid - 1;
        }
    ...
    l = i, r = n << 1;
    while(l <= r) {
        if(vec[(mid = l + r >> 1) - 1].x - vec[i-1].x < m) {
            l = mid + 1;
        } else {
            r = mid - 1;
        }
...
int main() {
    ...
    for(int i = 1; i <= n; i++) {
        int xi, yi;
        cin >> xi >> yi;
        vec.push_back(node(xi, i));
        vec.push_back(node(yi, i+n));
    }
