...
int main(){
	...
	scanf("%s",s+1);
rep(i,1,n) a[i]=s[i]-'0'; //changed from a[i]=s[n+1-i]-'0'
	scanf("%s",s+1);
	rep(i,1,m) b[i]=s[i]-'0'; //changed from b[i]=s[m+1-i]-'0'
	per(i,max(n,m),1){
		if(a[i]&&b[i]) p[++cnt]=i;
	...
	int la=max(n,m)+K,lb=max(n,m)+K; //changed from int la=n+K,lb=n+K;
	while(!ra[la]) la--;
	while(!rb[lb]) lb--;
