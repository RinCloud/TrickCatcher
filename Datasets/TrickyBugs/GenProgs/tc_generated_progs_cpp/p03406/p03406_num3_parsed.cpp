...
int main(){
	...
	scanf("%d%d",&n,&m); lor(i,1,m) scanf("%d",&a[i]); sort(a+1,a+1+m,cmp);
	fac[0]=1; lor(i,1,mov(n)-1) fac[i]=1ll*fac[i-1]*i%MOD;
	inv[mov(n)-1]=qsm(fac[mov(n)-1],MOD-2); ror(i,0,mov(n)-2) inv[i]=1ll*inv[i+1]*(i+1)%MOD;
	dp[0][0]=1;
	...
	f=1ll*f*mov(n)%MOD;				// fix 1:  muliply f with a constant
	printf("%d\n",f);
