...
int main() {
    ...
    n = strlen(s + 1), m = read();
    s[++n] = '0';
    m = min(n, m);      // fixed bug: m should be at most n, not gu
    for(int i = 1; i <= n; ++i) {      // fixed bug: start from i = 1
        if(s[i] == '0')++ gu;
        else ++ a[gu + 1];
    }
    for(int i = 1; i <= gu; ++ i) {
    ...
    for(int i = 1; i <= gu; ++ i) {
        ...
        for(int j = sum[i]; j <= n - gu; ++ j) {
            ...
            if(lim >= a[i]) {       // fixed bug: lim should be >= a[i], not >
                for(int k = 0; k <= min(j, m); ++ k) {     // fixed bug: k should be <= min(j, m), not <
                    dp[i][j][k] = 1LL * (s1[j - (a[i] + 1)][k] - s1[j - lim - 1][k] + mod) % mod;
                }
            }
            for(int k = 0; k <= min(j, m); ++ k) {     // fixed bug: k should be <= min(j, m), not <
                dp[i][j][k] = (dp[i][j][k] + s2[j + k - a[i]][j - max(0, a[i] - k)]) % mod;
                dp[i][j][k] = (dp[i][j][k] < 0 ? dp[i][j][k] + mod : dp[i][j][k]);
            }
        }
    }
    for(int k = 0; k <= m; ++ k) {       // fixed bug: k should be <= m, not <
