...
int x[200001], y[200001], lsh[200001], cnt;
ll ans=0;
int n;
...
int main() {
	...
	for(int i = 1; i <= n; i++) {
		add2(p[i].y, -1);
		int A1 = query(p[i].y);
		int B1 = query(n) - query(p[i].y);
		int C1 = query2(p[i].y);
		int D1 = query2(n) - query2(p[i].y);
		...
		ans = (ans + ((A2 - 1) * B2 % mod * C2 % mod * (D2 - 1) % mod)) % mod;
