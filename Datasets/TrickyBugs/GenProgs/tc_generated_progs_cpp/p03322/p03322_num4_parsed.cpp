...
{
	a%=mo,b%=mo;
	return (a*b-(long long int)((long double)a/mo*b+0.5)*mo+mo)%mo;
}
...
int main()
{
	...
	lint tar=0;
	for(int i=1;i<=n;i++)tar=trans(tar,s[i]);
	cnt[0]=1;
	lint ans=0,now=0;
	for(int i=1;i<=n;i++)
		now=trans(now,s[i]),ans+=cnt[sfx(now-tar+mo,loc[n-i])],cnt[sfx(now,loc[n-i])]++;
