...
void solve() {
    ...
    REP(i, n) {
        ...
        pair<pii, int> pl = MP(MP(x - s, y + s), 0), pu = MP(MP(x + s + 1, y + s), n);
        ...
        if (u > l) {
            ...
            deg[v] += u - l;
            ++deg[ps[l].second];
            ++imos[l]; --imos[u]; // corrected indices
        }
    }
    REP(i, n) {
        if (imos[i] > 0) {
...
int main(int argc, char **argv) {
    ...
    cout << (res - 2 * n) << endl; // subtract twice the number of pins to get correct output
