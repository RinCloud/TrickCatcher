...
int main()
{
    ...
    while(l<=r)
    {
        ...
        {
            for(int i=1;i<=n;i++)
                if(i<=p)buf=max(buf,s+(q>a[i].b?a[i].b*a[i].l+(q-a[i].b)*a[i].r:q*a[i].l)+a[p+1].sum-a[i].sum);
                else buf=max(buf,s+(q>a[i].b?a[i].b*a[i].l+(q-a[i].b)*a[i].r:q*a[i].l)+a[p].sum-a[i].sum);
        }
        if(buf>=o)r=m-1,ans=m;
        else l=m+1;
    }
    cout<<ans<<'\n';
}
