// Solution to the problem "Substring Counting"
...
int main() {
    ...
    // Ensure that the length of s is sufficiently large
    int exsize = n + m << 1;
    while ((int)s.size() < exsize) s += pat;
    s += pat, n = (int)s.size();
    
    // Compute the length of the maximum prefix substring
    ...
    // Check if the number of non-negative integers is finite or infinite
    if (res >= n / m - 2) return puts("-1") & 0;
    cout << res << endl;
    return 0;
}
